...
<body class="bg-[#F8F8E6] text-[#5B5C35] antialiased min-h-screen flex flex-col">
    ...
    <nav class="w-full py-6 px-12 flex items-center justify-between bg-transparent">
        ...
        <div class="flex space-x-8 font-semibold text-sm">
            <a href="<?php echo e(route('donasi.daftar')); ?>" class="text-[#5B5C35] border-b-2 border-[#5B5C35] pb-1">Beranda</a>
...
<?php endif; ?>        </div>
    ...
    <header class="px-12 py-10">
        ...
        <div class="flex space-x-3 overflow-x-auto pb-4">
            <a href="<?php echo e(route('donasi.daftar')); ?>"
               class="px-8 py-2.5 rounded-full font-bold text-sm transition <?php echo e(empty($kategori ?? '') ? 'bg-[#FCD34D] text-[#5B5C35] shadow-sm' : 'bg-[#E4E5C8] text-gray-500 hover:bg-[#D4D5B8]'); ?>">
               Semua
            </a>
            <a href="<?php echo e(route('donasi.filter', ['kategori' => 'Makanan'])); ?>"
               class="px-8 py-2.5 rounded-full font-bold text-sm transition <?php echo e(($kategori ?? '') === 'Makanan' ? 'bg-[#FCD34D] text-[#5B5C35] shadow-sm' : 'bg-[#E4E5C8] text-gray-500 hover:bg-[#D4D5B8]'); ?>">
               Makanan
            </a>
            <a href="<?php echo e(route('donasi.filter', ['kategori' => 'Snack'])); ?>"
               class="px-8 py-2.5 rounded-full font-bold text-sm transition <?php echo e(($kategori ?? '') === 'Snack' ? 'bg-[#FCD34D] text-[#5B5C35] shadow-sm' : 'bg-[#E4E5C8] text-gray-500 hover:bg-[#D4D5B8]'); ?>">
               Snack
            </a>
            <a href="<?php echo e(route('donasi.filter', ['kategori' => 'Minuman'])); ?>"
               class="px-8 py-2.5 rounded-full font-bold text-sm transition <?php echo e(($kategori ?? '') === 'Minuman' ? 'bg-[#FCD34D] text-[#5B5C35] shadow-sm' : 'bg-[#E4E5C8] text-gray-500 hover:bg-[#D4D5B8]'); ?>">
               Minuman
            </a>
        </div>
    </header>
    ...
    <main class="px-12 pb-20 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <?php $__empty_1 = true; $__currentLoopData = $donasis; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        <div class="bg-white rounded-[32px] overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300 border border-gray-100 flex flex-col">
            ...
            <div class="relative h-60">
                <img src="<?php echo e($item->foto_url ?: 'https://images.unsplash.com/photo-1555939594-58d7cb561ad1?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80'); ?>" class="w-full h-full object-cover" alt="<?php echo e($item->nama_makanan); ?>">
            ...
            <div class="p-6 flex-grow">
                <div class="flex justify-between items-start mb-4">
                    <h3 class="text-xl font-bold text-gray-800"><?php echo e($item->nama_makanan); ?></h3>
                ...
                <a href="<?php echo e(route('donasi.tambah')); ?>" class="block w-full py-3 bg-[#FCD34D] text-[#5B5C35] text-center font-bold text-sm rounded-2xl hover:bg-[#fbc316] transition-colors mb-2">
                    + Tambah Donasi Serupa
                </a>
                <?php endif; ?>
                <a href="<?php echo e(route('donasi.pesan.form', $item->id_donasi)); ?>" class="block w-full py-3 bg-[#E4E5C8] text-gray-600 text-center font-bold text-sm rounded-2xl hover:bg-[#D4D5B8] transition-colors">
                    Lihat Detail
                </a>
            </div>
        </div>
...
</html><?php /**PATH D:\SI4706-KELA\SI4706-KELA\resources\views/donasi/daftar.blade.php ENDPATH**/ ?>