    ...
    <aside class="w-64 bg-white border-r border-gray-100 flex flex-col justify-between py-6 shrink-0">
        <div>
            ...
            <nav class="space-y-1">
                ...
                <a href="<?php echo e(route('admin.pemberitahuan')); ?>" class="flex items-center px-8 py-3 bg-[#FCF8E3] text-[#6B630C] border-r-4 border-[#FCD34D] font-bold text-sm">
            ...
            <div class="flex gap-8 flex-1">
                ...
                <div class="w-[400px] shrink-0">
                    <div class="bg-white rounded-[32px] p-8 shadow-sm">
                        ...
                        <form action="<?php echo e(route('admin.pemberitahuan.store')); ?>" method="POST" class="space-y-4">
                            <?php echo csrf_field(); ?>
                            <div>
                    ...
                    <div class="bg-white rounded-[32px] p-8 shadow-sm flex-1 flex flex-col">
                        ...
                        <div id="announcements-list" class="space-y-4 flex-1 overflow-y-auto max-h-[500px] pr-2">
                            <?php $__empty_1 = true; $__currentLoopData = $announcements; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ann): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                ...
                                ?>
                                <div id="ann-card-<?php echo e($ann->id_pemberitahuan); ?>" class="ann-card bg-gray-50 rounded-2xl p-5 border border-gray-100 flex items-start justify-between gap-4 transition-all duration-300">
                                    <div class="flex items-start gap-4">
                                        <div class="w-12 h-12 rounded-2xl flex items-center justify-center text-xl shrink-0 <?php echo e($isMaintenance ? 'bg-amber-50' : 'bg-blue-50'); ?>">
                                            <?php echo e($emoji); ?>

                                        </div>
                                        <div>
                                            <div class="flex items-center gap-3 mb-1">
                                                <h4 class="font-extrabold text-sm text-gray-800"><?php echo e($ann->judul); ?></h4>
                                                <span class="text-[10px] font-bold px-2.5 py-0.5 rounded-full border uppercase tracking-wider <?php echo e($badgeBg); ?>"><?php echo e($ann->tipe); ?></span>
                                            </div>
                                            <p class="text-xs text-gray-500 font-medium leading-relaxed mb-3"><?php echo e($ann->pesan); ?></p>
                                            
                                            <div class="flex gap-4 text-[10px] font-bold text-gray-400 uppercase tracking-wide">
                                                <?php if($ann->tanggal_mulai): ?>
                                                    <span class="flex items-center"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg> Mulai: <?php echo e(\Carbon\Carbon::parse($ann->tanggal_mulai)->format('d M Y H:i')); ?></span>
                                                <?php endif; ?>
                                                <?php if($ann->tanggal_selesai): ?>
                                                    <span class="flex items-center"><svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg> Selesai: <?php echo e(\Carbon\Carbon::parse($ann->tanggal_selesai)->format('d M Y H:i')); ?></span>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                    </div>
                                    
                                    <form action="<?php echo e(route('admin.pemberitahuan.destroy', $ann->id_pemberitahuan)); ?>" method="POST" class="delete-ann-form">
                                        <?php echo csrf_field(); ?>
...
<?php /**PATH D:\SI4706-KELA\SI4706-KELA\resources\views/admin/pemberitahuan.blade.php ENDPATH**/ ?>