...
<?php endif; ?>
    ...
    <main class="flex-grow max-w-5xl mx-auto px-4 sm:px-8 w-full py-8">
        ...
        <div class="flex space-x-3 mb-10">
            <a href="<?php echo e(route('penerima.riwayatpenerimaan')); ?>" class="<?php echo e(empty($statusAktif) ? 'bg-[#FCD34D] text-[#5B5C35] shadow-sm' : 'bg-[#EEF0D5] text-[#85884B] hover:bg-[#e4e7c4]'); ?> transition px-6 py-2 rounded-full font-bold text-sm">Semua</a>
            <a href="<?php echo e(route('penerima.riwayatpenerimaan', ['status' => 'Disetujui'])); ?>" class="<?php echo e($statusAktif == 'Disetujui' ? 'bg-[#FCD34D] text-[#5B5C35] shadow-sm' : 'bg-[#EEF0D5] text-[#85884B] hover:bg-[#e4e7c4]'); ?> transition px-6 py-2 rounded-full font-bold text-sm">Disetujui</a>
            <a href="<?php echo e(route('penerima.riwayatpenerimaan', ['status' => 'Pending'])); ?>" class="<?php echo e(($statusAktif == 'Pending' || $statusAktif == 'Menunggu') ? 'bg-[#FCD34D] text-[#5B5C35] shadow-sm' : 'bg-[#EEF0D5] text-[#85884B] hover:bg-[#e4e7c4]'); ?> transition px-6 py-2 rounded-full font-bold text-sm">Menunggu</a>
        </div>
        ...
        <div class="space-y-6">
            <?php $__empty_1 = true; $__currentLoopData = $riwayatPenerimaan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="bg-white p-3 rounded-[32px] shadow-sm flex flex-col md:flex-row gap-6">
                    ...
                    <div class="w-full md:w-64 h-48 relative shrink-0">
                        <img src="<?php echo e($item->donasi->foto_url ?: 'https://images.unsplash.com/photo-1555939594-58d7cb561ad1?ixlib=rb-4.0.3'); ?>" alt="<?php echo e($item->donasi->nama_makanan ?? 'Makanan'); ?>" class="w-full h-full object-cover rounded-[24px]">
                        <div class="absolute top-3 left-3 bg-white/90 backdrop-blur text-xs font-bold px-3 py-1.5 rounded-full text-[#5B5C35] shadow-sm">
                            <?php echo e($item->jumlah_permintaan ?? '-'); ?> Porsi
                        </div>
                    </div>
                    
                    <div class="flex-1 flex flex-col justify-center py-2 pr-4">
                        <div class="flex justify-between items-start mb-2">
                            <h2 class="text-xl font-bold text-gray-800"><?php echo e($item->donasi->nama_makanan ?? 'Nama Makanan'); ?></h2>
                            ...
                            <?php else: ?>
                                <span class="bg-[#FFEDD5] text-[#C2410C] text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1">
                                    ...
                                    <?php echo e($item->status == 'Pending' ? 'Menunggu' : ($item->status ?? 'Diproses')); ?>
                        ...
                        <div class="flex flex-wrap gap-x-6 gap-y-2 text-xs text-gray-500 font-medium mb-4">
                            <div>Kedaluwarsa: <?php echo e(\Carbon\Carbon::parse($item->donasi->tanggal_kadaluarsa)->format('d M, H:i')); ?></div>
                            <div>Lokasi: <?php echo e($item->donasi->lokasi->alamat ?? 'Bandung'); ?></div>
                            <div>Diminta: <span class="realtime-time" data-timestamp="<?php echo e($item->created_at->toIso8601String()); ?>"><?php echo e($item->created_at->diffForHumans()); ?></span></div>
                        </div>
                        ...
                        <div class="flex justify-between items-center mt-2">
                            <div class="flex flex-col gap-1">
                                <div class="flex items-center gap-2 text-sm font-semibold text-gray-600">
                                    ...
                                    <?php echo e($item->donasi->user->nama ?? 'Donatur'); ?>
                                ...
                                <?php if($item->rating): ?>
                                    <div class="flex items-center gap-0.5 ml-1">
                                        <?php for($star = 1; $star <= 5; $star++): ?>
                                            <svg class="w-4 h-4 <?php echo e($star <= $item->rating->nilai_rating ? 'text-[#FCD34D]' : 'text-gray-200'); ?>" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                                            </svg>
                                        <?php endfor; ?>
                                        <span class="text-xs text-gray-400 ml-1 font-medium"><?php echo e($item->rating->nilai_rating); ?>/5</span>
                                    </div>
                                <?php endif; ?>
                            </div>
                            <a href="<?php echo e(route('rating.create', $item->id_donasi)); ?>" class="text-[#85884B] font-bold text-sm flex items-center gap-1 hover:text-[#5B5C35]">
                                <?php echo e($item->rating ? 'Edit Rating' : 'Rating'); ?> <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                            </a>
                        </div>
                    </div>
                </div>
        ...
        <div class="mt-10 flex justify-center">
            <?php echo e($riwayatPenerimaan->links()); ?>

        </div>

    </main>
</body>
</html><?php /**PATH C:\SI4706-KELA\resources\views/penerima/riwayatpenerimaan.blade.php ENDPATH**/ ?>