...
<?php endif; ?>
    ...
    <main class="flex-grow flex flex-col items-center py-10 px-4">
        ...
        <div class="bg-[#EEF0D5] w-full max-w-2xl rounded-[40px] p-8 md:p-10 shadow-sm">
            <form action="<?php echo e(route('rating.store', $donasi->id_donasi)); ?>" method="POST">

                <?php echo csrf_field(); ?>
                
                <input type="hidden" name="id_permintaan" value="<?php echo e($permintaan->id_permintaan ?? ''); ?>">

                <?php if($errors->any()): ?>
                    <div class="bg-red-100 text-red-700 p-4 rounded-3xl mb-6 text-sm font-semibold border border-red-200 shadow-sm">
                ...
                <?php
                    $currentRating = $existingRating ? $existingRating->nilai_rating : old('rating', 0);
                    $textOptions = ["Sangat Buruk", "Kurang Baik", "Cukup", "Sangat Enak!", "Sempurna!"];
                    $currentText = $currentRating > 0 ? $textOptions[$currentRating - 1] : "Pilih Rating";
                ?>

                <!-- Food Item Info -->
                <div class="bg-white rounded-[30px] p-4 flex items-center space-x-5 mb-8 shadow-sm">
                    <img src="<?php echo e($donasi->foto_url ?: 'https://via.placeholder.com/150'); ?>" alt="<?php echo e($donasi->nama_makanan); ?>" class="w-24 h-24 rounded-2xl object-cover">
                    <div>
                        <h2 class="text-xl font-bold text-gray-800"><?php echo e($donasi->nama_makanan); ?></h2>
                        <div class="flex items-center text-xs text-gray-500 mt-1 space-x-1">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                            <span>Kategori : <?php echo e($donasi->kategori); ?></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="text-center mb-8">
                    ...
                    <div class="flex justify-center space-x-2" id="star-container">
                        <?php for($i = 1; $i <= 5; $i++): ?>
                            <svg data-value="<?php echo e($i); ?>" class="star w-12 h-12 cursor-pointer transition-colors duration-200 <?php echo e($i <= $currentRating ? 'text-[#7C7E3A] fill-current' : 'text-gray-300 fill-current'); ?>" viewBox="0 0 24 24">
                                <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"></path>
                            </svg>
                        <?php endfor; ?>
                    </div>
                    <p class="text-[#7C7E3A] font-bold mt-2" id="rating-text"><?php echo e($currentText); ?></p>
                    <input type="hidden" name="rating" id="rating-value" value="<?php echo e($currentRating); ?>">
                </div>
                ...
                <?php if($existingRating): ?>
                    <div class="w-full bg-green-100 text-green-700 font-bold py-4 rounded-full text-center mb-2">
                        ✅ Anda sudah memberikan rating <?php echo e($existingRating->nilai_rating); ?> bintang.
                    </div>
                <?php endif; ?>
                <button type="submit" class="w-full bg-[#FCD34D] hover:bg-[#fbc629] transition duration-200 text-[#5B5C35] font-bold py-4 rounded-full shadow-sm">
                    <?php echo e($existingRating ? 'Perbarui Ulasan' : 'Kirim Ulasan'); ?>
...
</html><?php /**PATH C:\SI4706-KELA\resources\views/penerima/rating.blade.php ENDPATH**/ ?>