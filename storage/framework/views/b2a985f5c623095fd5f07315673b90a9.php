<div class="flex items-center space-x-6 text-[#85884B]">
    ...
    <div class="relative flex items-center" id="notif-wrapper">
        <button type="button" onclick="document.getElementById('notif-dropdown').classList.toggle('hidden'); event.stopPropagation();" class="flex items-center justify-center w-10 h-10 relative text-gray-500 hover:text-[#5B5C35] transition-colors">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
            <?php if($notifCount > 0): ?>
                <span id="notif-badge" class="absolute top-0.5 right-0.5 min-w-[18px] h-[18px] bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center px-1 shadow"><?php echo e($notifCount); ?></span>
            <?php else: ?>
                <span id="notif-badge" class="hidden absolute top-0.5 right-0.5 min-w-[18px] h-[18px] bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center px-1 shadow"></span>
            <?php endif; ?>
        </button>

        <!-- Dropdown Panel -->
        <div id="notif-dropdown" class="hidden absolute right-0 top-full mt-3 w-80 bg-white rounded-3xl shadow-2xl border border-gray-100 z-50 overflow-hidden" style="box-shadow: 0 20px 60px rgba(0,0,0,0.12);">
            ...
            <div id="notif-list" class="px-3 pb-4 space-y-1 max-h-72 overflow-y-auto">
                ...
                ?>
                <a href="<?php echo e($notif->id_permintaan ? route('notifikasi.redirect', $notif->id_notifikasi) : 'javascript:void(0)'); ?>" onclick="<?php echo e($notif->id_permintaan ? '' : 'markAsRead(' . $notif->id_notifikasi . ', this)'); ?>" class="notif-item flex items-start gap-3 px-3 py-3 rounded-2xl <?php echo e($notif->status_baca == 0 ? 'bg-[#FAFAEB] notif-unread' : ''); ?> hover:bg-gray-50 transition cursor-pointer">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-[10px] text-gray-400 mt-0.5 realtime-time" data-timestamp="<?php echo e($notif->created_at->toIso8601String()); ?>"><?php echo e($notif->created_at->diffForHumans()); ?></p>
                    </div>
    ...
    <!-- History Icon -->
    <?php if(!auth()->check() || trim(strtolower(auth()->user()->role)) !== 'admin'): ?>
        <?php
            $isRiwayatActive = request()->routeIs('penerima.riwayatpenerimaan') || request()->routeIs('donasi.riwayat');
            $riwayatRoute = (auth()->check() && auth()->user()->role === 'Penerima')
                ? route('penerima.riwayatpenerimaan')
                : route('donasi.riwayat');
            $riwayatTitle = (auth()->check() && auth()->user()->role === 'Penerima')
                ? 'Riwayat Penerimaan'
                : 'Riwayat Donasi';
        ?>
        <a href="<?php echo e($riwayatRoute); ?>" class="flex items-center justify-center w-10 h-10 rounded-full transition-colors <?php echo e($isRiwayatActive ? 'text-[#FCD34D]' : 'text-gray-500 hover:text-[#5B5C35]'); ?>" title="<?php echo e($riwayatTitle); ?>">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
        </a>
    <?php endif; ?>
    <!-- User Profile -->
    <?php if(auth()->check()): ?>
        <a href="<?php echo e(route('profile.show')); ?>" class="w-10 h-10 rounded-full border-2 border-[#FCD34D] overflow-hidden cursor-pointer flex items-center justify-center">
            <img src="<?php echo e(auth()->user()->foto_profil ? asset('storage/' . auth()->user()->foto_profil) : 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->nama ?? 'User') . '&background=FCD34D&color=5B5C35'); ?>" alt="Profile" class="w-full h-full object-cover">
        </a>
    <?php else: ?>
        <a href="<?php echo e(route('login')); ?>" class="text-sm font-semibold text-gray-500 hover:text-[#5B5C35]">Login</a>
    <?php endif; ?>
</div>

<?php if (! $__env->hasRenderedOnce('13d226c1-8e30-4b98-b3cd-869ab412cfb6')): $__env->markAsRenderedOnce('13d226c1-8e30-4b98-b3cd-869ab412cfb6'); ?>
<script>
...
<?php endif; ?>
<?php /**PATH D:\SI4706-KELA\SI4706-KELA\resources\views/components/navbar-icons.blade.php ENDPATH**/ ?>