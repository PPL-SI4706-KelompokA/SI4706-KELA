...
class TCFKM04Test extends DuskTestCase
{
    ...
    public function test_reset_menampilkan_semua_kategori()
    {
        $penerima = $this->getOrCreatePenerima('siti@email.com', 'Siti Aminah');
        
        // Buat donatur
        $donatur = User::where('email', 'donatur_fkm04@email.com')->first();
        if (!$donatur) {
            $donatur = User::create([
                'nama' => 'Donatur FKM04',
                'email' => 'donatur_fkm04@email.com',
                'password' => bcrypt('password'),
                'role' => 'Donatur',
        ...
        Donasi::where('nama_makanan', 'Es Kelapa FKM04')->delete();
        Donasi::where('nama_makanan', 'Nasi Liwet FKM04')->delete();
        ...
        $this->browse(function (Browser $browser) use ($penerima) {
            ...
            $browser->clickLink('Minuman')
                    ->pause(1000)
                    ->assertSee('Es Kelapa FKM04')
                    ->assertDontSee('Nasi Liwet FKM04');

            // 3. Klik opsi Semua Kategori
            $browser->clickLink('Semua')
                    ->pause(1000)
                    // 4. Pastikan kembali menampilkan seluruh donasi
                    ->assertSee('Es Kelapa FKM04')
                    ->assertSee('Nasi Liwet FKM04');
        });
    }
}
