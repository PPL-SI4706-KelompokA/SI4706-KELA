...
<body class="bg-[#F8F8E6] antialiased overflow-hidden">
    ...
    <!-- Overlay Navbar -->
    <nav class="absolute top-0 left-0 w-full py-6 px-12 flex items-center justify-between z-20 bg-[#F8F8E6] border-b border-gray-200/50 shadow-sm">
    ...
    <div class="absolute bottom-10 left-12 z-10 w-80" id="donation-info-card">
        <div class="bg-white rounded-[32px] overflow-hidden shadow-2xl border border-gray-100 transition-all transform hover:-translate-y-1">
            <div class="relative h-40">
                <img src="https://images.unsplash.com/photo-1555939594-58d7cb561ad1?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80" class="w-full h-full object-cover">
    ...
    <script>
        ...
        function showDonationDetail(donasi) {
            ...
            img.src = donasi.foto_url || 'https://images.unsplash.com/photo-1555939594-58d7cb561ad1?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80';
            ...
            if (donasi.tanggal_kadaluarsa) {
                const dateStr = donasi.tanggal_kadaluarsa.replace(' ', 'T');
                const expDate = new Date(dateStr);
                if (!isNaN(expDate.getTime())) {
                    formattedTime = expDate.toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' });
                }
            }
        ...
        if(dbDonasi && dbDonasi.length > 0) {
            dbDonasi.forEach(donasi => {
                if (donasi.lokasi) {
                    ...
                    marker.on('click', () => {
                        ...
                        map.panTo([donasi.lokasi.latitude, donasi.lokasi.longitude]);
                    });
                }
            });
            ...
            const cardElement = document.getElementById('donation-info-card');
