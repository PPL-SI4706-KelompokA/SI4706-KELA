                ...
                <div class="flex-1 flex flex-col justify-center py-2 pr-4">
                    ...
                    <div class="flex justify-between items-center mt-2">
                        <div class="flex items-center gap-2 text-sm font-semibold text-gray-600">
                            ...
                            @if($item->permintaans && $item->permintaans->count() > 0)
                                @php $latestReq = $item->permintaans->first(); @endphp
                                Penerima: {{ $latestReq->user->nama ?? 'Tidak diketahui' }}
                                @if($item->permintaans->count() > 1)
                                    <span class="text-xs text-gray-400">(+{{ $item->permintaans->count() - 1 }} lainnya)</span>
                                @endif
                            @else
                                Penerima: Menunggu...
                            @endif
    ...
    <script>
        function openDetailModal(donasiId) {
            ...
            if (status === 'Available' || status === 'Tersedia') {
                statusEl.className = 'text-xs font-bold px-3 py-1 rounded-full bg-[#D1FAE5] text-[#065F46]';
            } else if (status === 'Booked' || status === 'Dipesan') {
                statusEl.className = 'text-xs font-bold px-3 py-1 rounded-full bg-[#FEF3C7] text-[#92400E]';
            } else if (status === 'Distributed' || status === 'Selesai') {
                statusEl.className = 'text-xs font-bold px-3 py-1 rounded-full bg-[#DBEAFE] text-[#1E40AF]';
            } else {
                statusEl.className = 'text-xs font-bold px-3 py-1 rounded-full bg-gray-100 text-gray-600';
            }
