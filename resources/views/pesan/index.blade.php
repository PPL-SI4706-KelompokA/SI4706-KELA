        ...
        @if(auth()->user()->role !== 'Admin' && auth()->user()->role !== 'admin')
            ...
            <nav class="w-full py-6 px-12 flex items-center justify-between shrink-0 bg-transparent">
                ...
                <div class="hidden md:flex space-x-8 font-semibold text-sm">
                    ...
                    <a href="{{ route('pesan.index') }}" class="text-[#5B5C35] border-b-2 border-[#5B5C35] pb-1">Pesan</a>
                </div>
                <x-navbar-icons />
            </nav>
        @else
            ...
            <header class="flex items-center justify-between px-10 py-6 shrink-0 bg-transparent">
                ...
                <div class="flex items-center space-x-4 text-gray-400">
                    ...
                    <a href="{{ route('profile.show') }}">
                        <img src="{{ auth()->user()->foto_profil ? asset('storage/' . auth()->user()->foto_profil) : 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->nama) . '&background=FCD34D&color=6B630C&size=128' }}" class="w-8 h-8 rounded-full object-cover border-2 border-white shadow-sm">
                    </a>
                </div>
            </header>
        @endif
            ...
            <div class="w-80 bg-white rounded-[32px] p-6 shadow-sm border border-gray-100 flex flex-col shrink-0">
                ...
                <div class="flex-1 overflow-y-auto chat-scroll space-y-2" id="contacts-container">
                    @forelse($contacts as $contact)
                        <div class="contact-item flex items-center justify-between p-3.5 rounded-2xl cursor-pointer hover:bg-[#F8F8EC]/50 transition duration-200 border border-transparent {{ $activeContact && $activeContact->id_user == $contact->id_user ? 'bg-[#FCF8E3] border-[#FCD34D]/30 active-contact' : '' }}" 
                             data-id="{{ $contact->id_user }}"
                             data-nama="{{ $contact->nama }}"
                             data-role="{{ $contact->role }}"
                             data-avatar="{{ $contact->foto_profil ? asset('storage/' . $contact->foto_profil) : 'https://ui-avatars.com/api/?name=' . urlencode($contact->nama) . '&background=FCD34D&color=6B630C&size=128' }}">
                            <div class="flex items-center min-w-0 flex-1">
                                <img src="{{ $contact->foto_profil ? asset('storage/' . $contact->foto_profil) : 'https://ui-avatars.com/api/?name=' . urlencode($contact->nama) . '&background=FCD34D&color=6B630C&size=128' }}" class="w-11 h-11 rounded-full object-cover mr-3 shrink-0">
                                <div class="min-w-0 flex-1">
                                    <div class="flex justify-between items-start mb-0.5">
                                        <h4 class="text-sm font-bold text-gray-800 truncate mr-1">{{ $contact->nama }}</h4>
                                        <span class="text-[8px] font-bold text-gray-400 uppercase tracking-wide px-1.5 py-0.5 rounded bg-gray-50 border border-gray-100 shrink-0">{{ $contact->role }}</span>
                                    </div>
                                    <p class="text-xs text-gray-400 truncate font-semibold pr-4">
                                        {{ $contact->last_message ? $contact->last_message->pesan : 'Mulai chat baru...' }}
                                    </p>
                                </div>
                            </div>
                            <!-- Unread dot or counter -->
                            @if($contact->unread_count > 0)
                                <span class="w-5 h-5 bg-[#D97706] text-white text-[9px] font-bold rounded-full flex items-center justify-center shrink-0 ml-1 shadow-sm">{{ $contact->unread_count }}</span>
                            @endif
                        </div>
                    @empty
                ...
                @if($activeContact)
                    ...
                    <div class="px-8 py-5 border-b border-gray-50 flex items-center justify-between shrink-0">
                        <div class="flex items-center">
                            <img id="chat-header-avatar" src="{{ $activeContact->foto_profil ? asset('storage/' . $activeContact->foto_profil) : 'https://ui-avatars.com/api/?name=' . urlencode($activeContact->nama) . '&background=FCD34D&color=6B630C&size=128' }}" class="w-12 h-12 rounded-full object-cover mr-4">
                            <div>
                                <h3 id="chat-header-nama" class="text-base font-extrabold text-gray-800 leading-tight">{{ $activeContact->nama }}</h3>
                                <span id="chat-header-role" class="text-[9px] font-extrabold text-[#6B630C] bg-[#FCF8E3] border border-[#FCD34D]/30 px-2.5 py-0.5 rounded-full uppercase tracking-wider block mt-1 w-max">{{ $activeContact->role }}</span>
                            </div>
                        </div>
                    ...
                    <form id="chat-send-form" class="p-6 border-t border-gray-50 flex items-center gap-4 shrink-0 bg-white">
                        @csrf
                        <input type="hidden" name="id_penerima" id="chat-recipient-id" value="{{ $activeContact->id_user }}">
