            ...
            <form action="{{ route('rating.store', $donasi->id_donasi) }}" method="POST">
                ...
                <div class="text-center mb-8">
                    ...
                    <div class="flex justify-center space-x-2" id="star-container">
                        @for ($i = 1; $i <= 5; $i++)
                            <svg data-value="{{ $i }}" class="star w-12 h-12 cursor-pointer transition-colors duration-200 {{ $i <= $currentRating ? 'text-[#7C7E3A] fill-current' : 'text-gray-300 fill-current' }}" viewBox="0 0 24 24">
                                <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"></path>
                            </svg>
                        @endfor
                    </div>
                    <p class="text-[#7C7E3A] font-bold mt-2" id="rating-text">{{ $currentText }}</p>
                    <input type="hidden" name="rating" id="rating-value" value="{{ $currentRating }}">
                </div>
