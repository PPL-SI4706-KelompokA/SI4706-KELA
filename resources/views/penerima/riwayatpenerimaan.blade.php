...
<body class="bg-[#F8F8E6] text-[#5B5C35] antialiased min-h-screen flex flex-col">
    ...
    <main class="flex-grow max-w-5xl mx-auto px-4 sm:px-8 w-full py-8">
        ...
        <div class="flex space-x-3 mb-10">
            ...
            <a href="{{ route('penerima.riwayatpenerimaan', ['status' => 'Disetujui']) }}" class="{{ $statusAktif == 'Disetujui' ? 'bg-[#FCD34D] text-[#5B5C35] shadow-sm' : 'bg-[#EEF0D5] text-[#85884B] hover:bg-[#e4e7c4]' }} transition px-6 py-2 rounded-full font-bold text-sm">Disetujui</a>
            <a href="{{ route('penerima.riwayatpenerimaan', ['status' => 'Pending']) }}" class="{{ ($statusAktif == 'Pending' || $statusAktif == 'Menunggu') ? 'bg-[#FCD34D] text-[#5B5C35] shadow-sm' : 'bg-[#EEF0D5] text-[#85884B] hover:bg-[#e4e7c4]' }} transition px-6 py-2 rounded-full font-bold text-sm">Menunggu</a>
        </div>
                        ...
                        <div class="flex justify-between items-start mb-2">
                            ...
                            @else
                                <span class="bg-[#FFEDD5] text-[#C2410C] text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1">
                                    ...
                                    {{ $item->status == 'Pending' ? 'Menunggu' : ($item->status ?? 'Diproses') }}
                                </span>
                            @endif
                        </div>
