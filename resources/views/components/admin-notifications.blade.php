...
<div class="relative" id="admin-notif-wrapper">
    <button type="button" onclick="document.getElementById('admin-notif-dropdown').classList.toggle('hidden'); event.stopPropagation();" class="relative text-gray-400 hover:text-[#6B630C] transition-colors bg-transparent border-none outline-none cursor-pointer p-1">
        ...
        @if($notifCount > 0)
            <span id="admin-notif-badge" class="absolute -top-1 -right-1 min-w-[16px] h-[16px] bg-red-500 text-white text-[8px] font-bold rounded-full flex items-center justify-center px-1 shadow">{{ $notifCount }}</span>
        @else
    ...
    <!-- Dropdown Panel -->
    <div id="admin-notif-dropdown" class="hidden absolute right-0 mt-3 w-80 bg-white rounded-3xl shadow-2xl border border-gray-100 z-50 overflow-hidden" style="box-shadow: 0 20px 60px rgba(0,0,0,0.12); color: #333;">
            ...
            @php
                ...
                $redirectUrl = $notif->id_permintaan 
                    ...
                    : (($notif->tipe_notifikasi === 'Permintaan Baru' && auth()->user() && auth()->user()->role === 'Admin') 
                        ...
                        : 'javascript:void(0)');
            @endphp
            <a href="javascript:void(0)" onclick="markAdminAsRead({{ $notif->id_notifikasi }}, this, '{{ $redirectUrl }}')" class="admin-notif-item flex items-start gap-3 px-3 py-3 rounded-2xl {{ $notif->status_baca == 0 ? 'bg-[#FCF8E3]/40 admin-notif-unread' : '' }} hover:bg-gray-50 transition cursor-pointer">
                <div class="w-9 h-9 {{ $icon['bg'] }} rounded-xl flex items-center justify-center text-base flex-shrink-0">
                    {{ $icon['emoji'] }}
                </div>
                <div class="flex-1 min-w-0">
                    <p class="text-xs font-semibold text-gray-800 leading-snug">{{ $notif->pesan }}</p>
                    <p class="text-[9px] text-gray-400 mt-0.5 realtime-time" data-timestamp="{{ $notif->created_at->toIso8601String() }}">{{ $notif->created_at->diffForHumans() }}</p>
                </div>
        ...
        <div class="p-4 bg-gray-50 text-center border-t border-gray-100">
            <a href="{{ route('admin.verifikasi') }}" class="text-[10px] font-bold text-[#6B630C] hover:underline uppercase tracking-wider">Buka Menu Verifikasi</a>
        </div>
    </div>
</div>
...
<script>
    ...
    function markAdminAsRead(id, element, redirectUrl) {
        if (!element.classList.contains('admin-notif-unread')) {
            ...
            .then(data => {
                ...
                if (list && data.items.length > 0) {
                    list.innerHTML = data.items.map(n => {
                        ...
                        return `<a href="javascript:void(0)" onclick="markAdminAsRead(${n.id_notifikasi}, this, '${redirectUrl}')" class="admin-notif-item flex items-start gap-3 px-3 py-3 rounded-2xl ${unread ? 'bg-[#FCF8E3]/40 admin-notif-unread' : ''} hover:bg-gray-50 transition cursor-pointer">
