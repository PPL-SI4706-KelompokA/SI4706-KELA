<div class="flex items-center space-x-6 text-[#85884B]">
    ...
    <div class="relative flex items-center" id="notif-wrapper">
        <button type="button" onclick="document.getElementById('notif-dropdown').classList.toggle('hidden'); event.stopPropagation();" class="flex items-center justify-center w-10 h-10 relative text-gray-500 hover:text-[#5B5C35] transition-colors">
            ...
            @if($notifCount > 0)
                <span id="notif-badge" class="absolute top-0.5 right-0.5 min-w-[18px] h-[18px] bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center px-1 shadow">{{ $notifCount }}</span>
            @else
    ...
    @if(!auth()->check() || trim(strtolower(auth()->user()->role)) !== 'admin')
        @php
            ...
            $riwayatTitle = (auth()->check() && auth()->user()->role === 'Penerima')
                ...
                : 'Riwayat Donasi';
        @endphp
        <a href="{{ $riwayatRoute }}" class="flex items-center justify-center w-10 h-10 rounded-full transition-colors {{ $isRiwayatActive ? 'text-[#FCD34D]' : 'text-gray-500 hover:text-[#5B5C35]' }}" title="{{ $riwayatTitle }}">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
        </a>
    @endif
    <!-- User Profile -->
    @if(auth()->check())
        <a href="{{ route('profile.show') }}" class="w-10 h-10 rounded-full border-2 border-[#FCD34D] overflow-hidden cursor-pointer flex items-center justify-center">
    ...
    @else
        <a href="{{ route('login') }}" class="text-sm font-semibold text-gray-500 hover:text-[#5B5C35]">Login</a>
    @endif
</div>
