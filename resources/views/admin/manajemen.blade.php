...
<body class="flex h-screen overflow-hidden">
    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto">
        ...
        <header class="flex items-center justify-between px-10 py-6">
            <form action="{{ route('admin.manajemen') }}" method="GET" class="relative w-96">
                @if(request('role'))
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search user by name or email..." class="w-full bg-white rounded-full py-3 pl-12 pr-6 text-sm font-medium border-none shadow-sm focus:ring-2 focus:ring-[#FCD34D] outline-none">
            </form>
        ...
        <div class="px-10 py-4 flex-1 flex flex-col">
            ...
            <div class="grid grid-cols-3 gap-6 mb-10">
                ...
                <div class="user-card bg-white rounded-[32px] p-6 shadow-sm border border-gray-50 hover:border-[#FCD34D] hover:shadow-md hover:scale-[1.01] transition-all cursor-pointer"
                     ...
                     data-status="{{ $user->status_verifikasi === 'Sudah Verifikasi' ? 'Aktif' : ($user->status_verifikasi === 'Diblokir' ? 'Diblokir' : 'Nonaktif') }}">
                        ...
                        <div class="relative">
                            ...
                            <div class="gear-dropdown hidden absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-xl border border-gray-100 py-1.5 z-20 transition-all duration-200">
                                @if($user->id_user != auth()->id())
                                    @if($user->status_verifikasi === 'Diblokir')
                                        <form action="{{ route('admin.manajemen.unban', $user->id_user) }}" method="POST" class="w-full m-0">
                                            @csrf
                                    ...
                                    @else
                                        <form action="{{ route('admin.manajemen.ban', $user->id_user) }}" method="POST" class="w-full m-0">
                                            @csrf
                    ...
                    <div class="flex justify-between items-center border-t border-gray-50 pt-4">
                        ...
                        @elseif($user->status_verifikasi === 'Diblokir')
