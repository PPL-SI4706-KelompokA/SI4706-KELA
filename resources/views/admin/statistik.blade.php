            ...
            <div class="grid grid-cols-3 gap-6 mb-6">
                ...
                <div class="col-span-2 bg-[#6B630C] rounded-[32px] p-10 text-white relative overflow-hidden">
                    ...
                    <div class="flex items-end gap-4 mb-12 relative z-10">
                        <h3 class="text-8xl font-extrabold leading-none">{{ number_format($totalDonasiTerkumpul, 0, ',', '.') }}</h3>
                    ...
                    <div class="grid grid-cols-2 gap-8 relative z-10 border-t border-white/20 pt-6">
                        <div>
                            ...
                            <p class="text-lg font-bold">{{ number_format($targetBulanan, 0, ',', '.') }} <span class="text-xs font-normal text-white/70">Paket</span></p>
                        </div>
                        <div>
                            <p class="text-[10px] font-bold text-white/50 tracking-widest uppercase mb-1">Status Keberhasilan</p>
                            <p class="text-lg font-bold">{{ $statusKeberhasilan }}%</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-span-1 bg-white rounded-[32px] p-8 shadow-sm flex flex-col justify-between">
                    ...
                    <div>
                        <h3 class="text-5xl font-extrabold text-gray-800 mt-6 mb-2">{{ number_format($totalDonatur, 0, ',', '.') }}</h3>
                        ...
                        <div class="flex -space-x-2">
                            @foreach($topDonators as $i => $donor)
                                @if($i < 3)
                                @php
                                ...
                                @endphp
                                @if(!empty($donor['foto_profil']))
                                    <img src="{{ asset('storage/' . $donor['foto_profil']) }}"
                                ...
                                @else
                                    <div class="w-8 h-8 rounded-full border-2 border-white flex items-center justify-center text-[9px] font-extrabold shrink-0"
                                         style="background-color: {{ $bubbleBg }}; color: {{ $bubbleText }};">
                                        {{ $bubbleInit }}
                                    </div>
                                @endif
                                @endif
            ...
            <div class="bg-[#EAEBCA] rounded-[32px] p-8">
                <div class="flex justify-between items-center mb-6 px-2">
                    ...
                    <div class="flex items-center gap-3">
                        <form method="GET" action="{{ route('admin.statistik') }}" class="flex items-center gap-3 bg-white rounded-full px-4 py-2 text-xs font-semibold shadow-sm border border-gray-100">
                            <div class="flex items-center gap-1.5">
                                <span class="text-gray-400 font-bold uppercase tracking-wider text-[9px]">Dari</span>
                                <input type="date" name="start_date" value="{{ request('start_date') }}" class="bg-transparent border-none p-0 text-gray-700 outline-none w-28 focus:ring-0 text-xs">
                            </div>
                            <div class="h-4 w-px bg-gray-200"></div>
                            <div class="flex items-center gap-1.5">
                                <span class="text-gray-400 font-bold uppercase tracking-wider text-[9px]">Sampai</span>
                                <input type="date" name="end_date" value="{{ request('end_date') }}" class="bg-transparent border-none p-0 text-gray-700 outline-none w-28 focus:ring-0 text-xs">
                            </div>
                            ...
                            @if(request('start_date') || request('end_date'))
                                <a href="{{ route('admin.statistik') }}" class="text-gray-400 hover:text-red-500 font-bold text-xs">Reset</a>
                            @endif
                        </form>
                        <a href="{{ route('admin.manajemen') }}" class="text-sm font-bold text-[#6B630C] hover:underline flex items-center shrink-0">Lihat Semua <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7"></path></svg></a>
                    </div>
                </div>
                <div class="space-y-3">
                    ...
                    @endphp
                    <div class="{{ $bgClass }} rounded-[20px] p-4 px-6 flex items-center">
                        <span class="text-2xl font-bold text-gray-200 w-12">{{ $donor['rank'] }}</span>

                        {{-- Avatar: real photo if uploaded, else colored initials --}}
                        @if($hasFoto)
                            <img src="{{ asset('storage/' . $donor['foto_profil']) }}"
                        ...
                        @else
                            <div class="w-11 h-11 rounded-full flex items-center justify-center mr-4 text-xs font-extrabold shrink-0"
                                 style="background-color: {{ $avatarStyle['bg'] }}; color: {{ $avatarStyle['text'] }};">
                                {{ $initials }}
                            </div>
                        @endif
                        <div class="flex-1">
