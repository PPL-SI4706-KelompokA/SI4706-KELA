...
<body class="flex h-screen overflow-hidden">
    ...
    <aside class="w-64 bg-white border-r border-gray-100 flex flex-col justify-between py-6 shrink-0">
        ...
        {{-- Removed Export Data, Support, and Sign Out footer --}}
    </aside>
    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto">
        ...
        <div class="px-10 py-4 flex-1 flex flex-col">
            ...
            <div class="flex gap-8 flex-1">
                ...
                <div class="flex-grow flex flex-col gap-6">
                    @if($selectedUser)
                    <!-- About -->
                    <div class="bg-white rounded-[40px] p-8 shadow-sm flex gap-4">
                        <div class="w-12 h-12 bg-[#F8F8EC] rounded-2xl flex items-center justify-center text-[#6B630C] shrink-0">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        </div>
                        <div>
                            <h3 class="text-xl font-extrabold text-gray-800 mb-3">Tentang Donatur</h3>
                            <p class="text-sm text-gray-500 font-medium leading-relaxed">"Saya mendaftar sebagai donatur untuk FoodShare. Saya ingin membagikan surplus makanan layak konsumsi dari usaha/pribadi untuk membantu saudara-saudara kita yang membutuhkan. Harap verifikasi dokumen saya agar saya dapat segera berdonasi."</p>
                        </div>
                    </div>

                    <!-- Map -->
                    <div class="bg-[#BBDCD2] rounded-[40px] h-48 relative overflow-hidden shadow-inner flex items-center justify-center bg-[url('https://www.transparenttextures.com/patterns/cartographer.png')]">
                        <div class="absolute left-8 bottom-8 bg-white rounded-full pl-2 pr-6 py-2 shadow-lg flex items-center gap-3">
                            <div class="w-10 h-10 bg-[#6B630C] rounded-full flex items-center justify-center text-white"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path></svg></div>
                            <div>
                                <p class="text-xs font-bold text-gray-800">{{ $selectedUser->alamat }}</p>
                                <p class="text-[10px] text-gray-400 font-medium">Bandung, Jawa Barat</p>
                            </div>
                        </div>
                        <div class="absolute w-4 h-4 bg-white rounded-full shadow-lg" style="left: 60%; top: 40%;"></div>
                    </div>

                    <!-- Documents -->
                    <div class="mb-2">
                        <p class="text-[10px] font-bold text-gray-400 tracking-widest uppercase mb-4 ml-2">DOKUMEN PENDUKUNG</p>
                        <div class="flex gap-4">
                            <div class="bg-white rounded-[20px] px-5 py-3 flex items-center gap-3 shadow-sm flex-1 cursor-pointer hover:bg-gray-50">
                                <svg class="w-5 h-5 text-[#6B630C]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                <span class="text-sm font-bold text-gray-700 flex-1">KTP_{{ str_replace(' ', '', $selectedUser->nama) }}.pdf</span>
                                <svg class="w-4 h-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                            </div>
                            <div class="bg-white rounded-[20px] px-5 py-3 flex items-center gap-3 shadow-sm flex-1 cursor-pointer hover:bg-gray-50">
                                <svg class="w-5 h-5 text-[#6B630C]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                                <span class="text-sm font-bold text-gray-700 flex-1">Izin_Usaha.pdf</span>
                                <svg class="w-4 h-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-auto bg-[#E8E9C9] rounded-full p-4 pl-8 flex items-center justify-between border border-[#D5D6A8] shadow-sm">
                        <p class="text-xs font-medium text-[#5B630C]">Pastikan seluruh dokumen telah diperiksa sebelum memberikan keputusan.</p>
                        <div class="flex gap-4">
                            ...
                            <form action="{{ route('admin.verifikasi.setuju', $selectedUser->id_user) }}" method="POST">
                                ...
                                <button type="submit" class="px-8 py-3 rounded-full bg-[#6B630C] text-white font-bold shadow-lg flex items-center border-none outline-none cursor-pointer hover:opacity-90">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg> Terima Donatur
                                </button>
                            </form>
                        </div>
                    </div>
        ...
        <button class="fixed bottom-8 right-8 w-14 h-14 bg-[#6B630C] text-white rounded-full shadow-2xl flex items-center justify-center hover:bg-[#524c08] transition-colors z-50">
