...
class RiwayatDonasiController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($statusFilter) {
            if (in_array(strtolower($statusFilter), ['tersedia', 'available'])) {
                $query->whereIn('status_donasi', ['Available', 'Booked']);
            } elseif (in_array(strtolower($statusFilter), ['habis', 'distributed'])) {
                $query->where('status_donasi', 'Distributed');
            } else {
